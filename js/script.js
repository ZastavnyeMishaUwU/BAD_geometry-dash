...
document.getElementById("textmapjson").value = JSON.stringify({
	music: "xtrullortria.mp3",
	obj: objects,
});
...
function gameloop() {
	if (isatmenu) {
		...
		for (let i = 0; i < dtl; i++) {
			...
			camy -=
				(camy - (playery - canvas.height + (canvas.height / 635) * 350)) /
				camspeed; //* (dt / normaltime)
...
function renderandcol(a) {
	falling = true;
	if (a) drawrectat(0, 0, canvas.width, canvas.height, "Black", 3, true);
	objects.forEach((i) => {
		if (i.Type == "Cube") {
...
document.body.addEventListener("keyup", function (e) {
	...
	if (e.key == "s") {
		...
		// Convert JSON object to string and minify it (remove spaces and newlines)
